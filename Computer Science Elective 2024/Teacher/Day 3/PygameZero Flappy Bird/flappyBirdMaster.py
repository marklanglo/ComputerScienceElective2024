...
class Pipe:
    ...
    def update(self, speed):
        self.actor_top.x -= speed
        self.actor_bot.x -= speed
...
#
#	1) Every time we start or restart the game, we reinitialize it with the first if
#		statement.  Within the first if statement we also start the spawner for the
#		duration of the games runtime.
#
#	2) We run the player update function from the class
#
#	3) We update the pipes
#
#	4) Gamestate actions, we check if the player earns points or loses
#
#=====================================================================================
def update():
    ...
    #1) We check if our scene has been (re)initialized, if not we initialize it
    ...
    #2) We check if a valid boolean for our player update is true, then if it is
    #	then we run the player.update() function from our class
    if player_ready or player.has_lost:
        player.update()
    
    #3) We check for a valid boolean and will update the pipes
    if player_ready:
        move_pipes()
        
    #4) Gamestate actions, we check if a set of pipes can add a point to the player, and
...
def check_collision():
    ...
    for pipe in pipes:
        if (player.check_collision(pipe.top_hitbox())
         or player.check_collision(pipe.bot_hitbox())
         or player.actor.y < -200
         or player.actor.y > HEIGHT - 50):
            ...
            #we check if we can update the high score
            if score > high_score:
                high_score = score
...
def on_key_down(key):
    ...
    if not player.has_lost:
        ...
        #1) Set player ready and Jump with 'up arrow', 'space', 'W', or 'J'
        if key in (keys.UP, keys.SPACE, keys.W, keys.J) and not player_ready:
            player.fly()
            player_ready = True
            
        #2) Jump with 'up arrow', 'space', 'W', or 'J'
        elif key in (keys.UP, keys.SPACE, keys.W, keys.J):
            player.fly()
            
    else:
        
        # 3) Reset the game with 'R'
        if key == keys.R:
